#Input an integer containing 0s and 1s (i.e., a “binary” integer) and display its decimal equivalent. The online appendix, “Number Systems,”
#discusses the binary number system. [Hint: Use the modulus and division operators to pick
#off the “binary” number’s digits one at a time from right to left. Just as in the decimal number system, where the rightmost digit has the positional value 1 and the next digit to the left
...
if(number > 9999 and number < 100000):
	...
	converted_number = (digit5 * 10) + (digit4 * 8) + (digit3 * 6) + (digit2 * 4) + (digit1 * 2)
